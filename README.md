...
4. 在**本 README 的「已收录规定」表**补一行、目录结构补一行
5. 若为**版本更新**：另需**全局搜索并更新失效引用**（旧条款 / 旧术语）

类别目录：`01-法律法规` / `02-民航规章CCAR` / `03-行业标准MH` / `04-规范性文件` / `05-ICAO`

## ⚠️ 说明

- 本技能为**条款定位与汇总**，不替代规范正本；具体量值（几何尺寸、坡度、光强、阈值等）与正式设计/审定/处罚认定，以发布正本及主管部门解释为准。
- 内容来源为文本可提取 PDF 的条文；图（几何图、布置图）未收录。部分复杂表格（如附件配置表）行列结构可能不完整，已在对应章节标注"以正本为准"。
